...
// Complete Observer Cpp implementation
...
public:
    explicit Observercpp(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Observercpp filter(F pred) const {
        Observercpp result;
...
int main() {
    cout << "=== Observer Cpp ===" << endl;

    Observercpp<int> container(20);
