...
// Complete Event System Cpp implementation

template<typename T>
class Eventsystemcpp {
    vector<T> data;
    size_t capacity;

public:
    explicit Eventsystemcpp(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Eventsystemcpp filter(F pred) const {
        Eventsystemcpp result;
...
int main() {
    cout << "=== Event System Cpp ===" << endl;

    Eventsystemcpp<int> container(20);
