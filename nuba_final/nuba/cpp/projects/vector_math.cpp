...
// Complete Vector Math implementation
...
public:
    explicit Vectormath(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Vectormath filter(F pred) const {
        Vectormath result;
...
int main() {
    cout << "=== Vector Math ===" << endl;

    Vectormath<int> container(20);
