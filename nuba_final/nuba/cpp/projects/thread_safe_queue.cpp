...
// Complete Thread Safe Queue implementation
...
public:
    explicit Threadsafequeue(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Threadsafequeue filter(F pred) const {
...
int main() {
    ...
    Threadsafequeue<int> container(20);
