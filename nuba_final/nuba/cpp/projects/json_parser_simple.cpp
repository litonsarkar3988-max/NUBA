...
// Complete Json Parser Simple implementation

template<typename T>
class Jsonparsersimple {
    vector<T> data;
    size_t capacity;

public:
    explicit Jsonparsersimple(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Jsonparsersimple filter(F pred) const {
        Jsonparsersimple result;
...
int main() {
    cout << "=== Json Parser Simple ===" << endl;

    Jsonparsersimple<int> container(20);
