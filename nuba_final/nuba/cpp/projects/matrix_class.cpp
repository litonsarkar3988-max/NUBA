...
// Complete Matrix Class implementation
...
public:
    explicit Matrixclass(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Matrixclass filter(F pred) const {
        Matrixclass result;
