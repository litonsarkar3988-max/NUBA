...
// Complete State Machine Cpp implementation
...
public:
    explicit Statemachinecpp(size_t cap = 16)
        : capacity(cap) {
        data.reserve(cap);
    }
    ...
    template<typename F>
    Statemachinecpp filter(F pred) const {
        Statemachinecpp result;
...
int main() {
    cout << "=== State Machine Cpp ===" << endl;

    Statemachinecpp<int> container(20);
